    ...
    public static class AccessRightsChecker
    {
        ...
        public static bool ItemHasPermision(string itemPath, FileSystemRights accessRight)
        {
            ...
            try
            {
                ...
                if (isDir == true)
                {
                    rules = new DirectoryInfo(itemPath).GetAccessControl().GetAccessRules(true, true, typeof(SecurityIdentifier));
                    //rules = Directory.GetAccessControl(itemPath).GetAccessRules(true, true, typeof(SecurityIdentifier)); 
                }
                else
                {
                    rules = new FileInfo(itemPath).GetAccessControl()/* File.GetAccessControl(itemPath)*/.GetAccessRules(true, true, typeof(SecurityIdentifier));
                }
